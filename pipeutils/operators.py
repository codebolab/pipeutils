...
class XComParams(BaseOperator):
    ...
    def __init__(self, lookups={}, *args, **kwargs):
        ...
        It uses the keys as param keys in self.dag_params and the values as the taskids.
        I.E::
            t = XComParams(lookups={'var1': 'taskid_1'})

        In function `execute` the `self.dag_params` var will be populated as a dictionary:
...
class UploadToS3(XComParams):
    ...
    def __init__(self, destination, extension, source=None, *args, **kwargs):
        """
        Operator to upload all files with file extension `extension` (if defined)
        in s3 from `source` directory to s3 path `destination`.

        if `source` or `destination` is None, they should be accesible through `dag_params`
        so `lookups` need to be explicit defined for that.
        Args:
            source (str): path to the directory to find the files to upload into s3.
    ...
    def execute(self, context):
        ...
        s3_client.upload_multiple(self.source, self.destination, self.extension)
        logger.info("The files has been saved successfully")


class PopulateDjango(XComParams):

    def __init__(self, django_settings, model_name, source=None, *args, **kwargs):
        """
        Populate data from a csv into a model in Django app.
        Args:
            django_settings (str): path to django settings.
            model_name (str): model name to store to Ex. `auth.User`
            source (str): path to csv file, in case of None the class should look at `dag_params`
        """
    ...
    def execute(self, context):
        ...
        if self.django_settings is None:
            self.django_settings = '$ANALITICS_PLATFORM/server/manage.py'

        if self.model_name is None:
            self.model_name = 'save_jobs_from_csv_to_django_database'

        cmd = "python {0} {1} --csv_path={2}".format(self.django_settings, self.model_name, self.source)

        sp = Popen(cmd, shell=True)
        sp.wait()
        logger.info("Command exited with return code {0}".format(sp.returncode))
...
class PopulateVertica(XComParams):
    ...
    def __init__(self, schema, table, source=None, *args, **kwargs):
        """
        Insert data into table `{schema}.{table}` all csv files found in source, 
        source should be accesible thourhg `self.dag_params` if not passed.
        Args:
    ...
    def execute(self, context):
        ...
        Vertica.insert_from_csv(client, self.schema, self.table, self.source, delimiter=';')
