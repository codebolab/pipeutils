...
class AvroJsonBase(object):
    ...
    def _serialize_bytes(self, schema, datum):
        ...
        for candidate_schema in schema.schemas:
            if validate(candidate_schema, datum):
                if candidate_schema.type == "null":
                    ...
                    return {
                        field_type_name: self._process_serializer(candidate_schema, datum)
                    }
        raise AvroTypeException(schema, datum)
    ...
    def _deserialize_record(self, schema, datum):
        ...
        for field in schema.fields:
            result[field.name] = self._process_deserializer(field.type,
                                                             datum.get(field.name, self.UNSET))
        return result

    def _desserialize_bytes(self, schema, datum):
